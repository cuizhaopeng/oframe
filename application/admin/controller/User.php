...
use app\admin\model\UnitUser as UnitUserModel;
use app\admin\model\Oauthority;
...
class User extends Controller
{
    ...
    {
        // 判断请求类型
//        (new UserValidate())->method()
        if(Request::isPost()){
            // 验证登录信息， 需要判断登录模式  1，手机登录。2，邮件登录
    ...
    {
        if(!Request::isPost()) return new MissException();
