    ...
    public function menuList()
    {
        ...
        $data = [
            ...
            ,"data"=>[
                ...
                [ //菜单数据，key名可以通过 config.js 去重新配置
                    "name"=>"analysis" //一级菜单名称（与视图的文件夹名称和路由路径对应）
                    ,"title"=>"对账单" //一级菜单标题
                    ,"icon"=>"layui-icon-component" //一级菜单图标样式
                    ,"jump"=>'/analysis/sheet/html' //自定义一级菜单路由地址，默认按照 name 解析。一旦设置，将优先按照 jump 设定的路由跳转
                    ,"spread"=>false //是否默认展子菜单（1.0.0-beta9 新增）
                    ,"list"=> [
                    /*[ //二级菜单
                        "name"=> "sheet" //二级菜单名称（与视图的文件夹名称和路由路径对应）
                        ,"title"=> "表格" //二级菜单标题
                        ,"jump"=> ''  //自定义二级菜单路由地址
                        ,"spread"=> true //是否默认展子菜单（1.0.0-beta9 新增）
                        ,"list"=> [
                        [ //三级菜单
                            "name"=> "html" //三级菜单名（与视图中最终的文件名和路由对应），如：component/grid/list
                            ,"title"=> "网页表格" //三级菜单标题
                        ]
                    ]*/
                    ]
